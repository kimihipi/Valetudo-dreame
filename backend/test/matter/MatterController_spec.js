...
describe("MatterController", function() {
    ...
    const createController = () => {
        ...
        controller.robot = {
            ...
            setCleaningTarget: target => {
                ...
                controller.robot.state.upsertFirstMatchingAttribute(new stateAttrs.CleaningTargetStateAttribute({
                    ...target,
    ...
    it("should emit one successful completion after cleaning returns to the dock", async function() {
        const controller = createController();
    ...
    it("should route selected Matter areas using the firmware room order", async function() {
        ...
        executedSegments.should.deepEqual([kitchen, bedroom]);
    ...
    it("should find the selected Matter room containing the robot", function() {
        ...
        controller.getCurrentServiceArea().should.equal(4);
    });
    ...
    it("should retain throttled map positioning updates while cleaning", function() {
        const controller = createController();
        let queuedOptions;
        controller.mapTopologyVersion = controller.getMapTopologyVersion();
        ...
        queuedOptions.should.deepEqual({rvc: true, serviceAreas: false, delayMs: 2000});
    });
