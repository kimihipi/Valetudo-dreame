...
const MapAnnotationActions = (
    ...
    const handleSaveClick = React.useCallback(() => {
        ...
        impassableThresholds.forEach((t) => {
            const pA = convertPixelCoordinatesToCMSpace({x: t.x0, y: t.y0});
            const pC = convertPixelCoordinatesToCMSpace({x: t.x1, y: t.y1});

            mapAnnotations.push({
                type: ValetudoMapAnnotationType.ImpassableThreshold,
                points: [pA, pC]
            });
        });
    ...
    }, [canEdit, saveAnnotations, thresholds, impassableThresholds, curtains, ramps, convertPixelCoordinatesToCMSpace]);
                        ...
                        <>
                            {
                                ...
                                >
                                    ...
                                    Passable threshold ({thresholds.length})
                                </ActionButton>
                            }
                            {
                                mapAnnotationsProperties.supportedAnnotationTypes.includes(ValetudoMapAnnotationType.ImpassableThreshold) &&
                                <ActionButton
                                    color="inherit"
                                    size="medium"
                                    variant="extended"
                                ...
                                >
                                    <AddThresholdIcon style={{marginRight: "0.25rem", marginLeft: "-0.25rem"}}/>
                                    Impassable threshold ({impassableThresholds.length})
                                </ActionButton>
                            }
                            {
            ...
            }
            {
                ...
                <Grid2>
                    <ActionButton
                        color="inherit"
                        size="medium"
                        disabled={thresholds.length === 0 && impassableThresholds.length === 0 && curtains.length === 0 && ramps.length === 0}
                        variant="extended"
                        onClick={onClear}
                    >
